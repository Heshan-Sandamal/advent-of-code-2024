...
print(row, column)
init_row, init_column = row, column

# Iterative traversal of the grid

def traverse_iterative(row, column, direction, grid):
    ...
    while stack:
        ...
        if 0 <= next_row < rows_length and 0 <= next_column < columns_length:
            ...
            if next_cell == ".":
                ...
                if (next_row == init_row and next_column == init_column and current_direction == (-1, 0)):
                    print("loop")
            elif next_cell == "#":
                # Turn right
                ...
                if 0 <= new_row < rows_length and 0 <= new_column < columns_length:
                    ...
                    if (new_row == init_row and new_column == init_column and new_direction == (-1, 0)):
                        print("loop")
...
for x in range(rows_length):
    for k in range(columns_length):
        ...
        new_grid[x][k] = "#"
        traverse_iterative(row, column, (-1, 0) , new_grid)
        print(x,k)
